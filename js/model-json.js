...
(function(exports) {
  ...
  function JSONMediaModel(appSettings) {
    ...
    this.loadInitialData = function(dataLoadedCallback) {
      var requestData = {
        ...
        success: function() {
          var jsonData = mapFaithpromiseData(arguments[0]);
        ...
        if (mediaCats) {
          for (var j = 0; j < mediaCats.length; j++) {
            ...
            if (catIndex < 0) {
              ...
              jsonData.folders[0].contents.push({
                type: "folder",
                id: categories.length
              });
  ...
  function mapFaithpromiseData(response) {
    var media = response.series.reduce(function(content, s) {
      var episodes = s.episodes.map(function(episode) {
        var thumbnail = episode.thumbnail.replace('&h=1080&w=1920', '&h=360&w=640')
        return {
          id: episode.id,
          title: episode.title,
      ...
      return content.concat(episodes);
    }, []);
    return {
      media: media
    };
